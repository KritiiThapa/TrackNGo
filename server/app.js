...
io.on("connection", (socket) => {
  ...
  if (currentDriverLocation) {
    ...
    console.log(
      "Sent stored driver location to new client:",
...
const school = { name: "School", lat: 27.7172, lng: 85.324 };
const pickupPoints = [
  { name: "Child 1 - School Gate", lat: 27.7172, lng: 85.324 },
  { name: "Child 2 - Park Area", lat: 27.72, lng: 85.32 },
  { name: "Child 3 - Main Road", lat: 27.715, lng: 85.328 },
  { name: "Child 4 - Community Center", lat: 27.722, lng: 85.322 },
];
  ...
  try {
    ...
    if (!driverLat || !driverLng || targetIndex === undefined) {
      return res.status(400).json({
        error: "Missing required parameters: driverLat, driverLng, targetIndex",
      });
    }
    ...
    const distance = calculateDistance(
      driverLocation.lat,
      driverLocation.lng,
      pickupPoints[targetIdx].lat,
      pickupPoints[targetIdx].lng
    );
    ...
    console.log(
      `📍 Calculated route: ${result.distance} km to ${pickupPoints[targetIdx].name}`
    );
    res.json(result);
  } catch (err) {
...
app.get("/api/road-route", async (req, res) => {
  try {
    ...
    const roadPath = routeData.routes[0].geometry.coordinates.map((coord) => [
      coord[1],
      coord[0],
    ...
    console.log(
      `🛣️ Road route calculated: ${distance} km, ${result.duration} min`
    );
    res.json(result);
  } catch (err) {
