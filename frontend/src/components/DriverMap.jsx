...
const DriverMap = () => {
  ...
  const calculateRouteToPoint = async (pointIndex) => {
    ...
    try {
      ...
      const newRouteLine = Leaflet.polyline(routeData.path, routeStyle).addTo(mapInstanceRef.current);
      ...
      const driverBounds = Leaflet.latLngBounds([driverLatLng, targetPoint]);
  ...
  const createFallbackRoute = (pointIndex) => {
    ...
    const fallbackRoute = Leaflet.polyline([
      [driverLatLng.lat, driverLatLng.lng],
      [targetPoint.lat, targetPoint.lng]
    ], {
      color: '#ef4444',
  ...
  const testSimpleRoute = (pointIndex) => {
    ...
    const testRoute = Leaflet.polyline([
      [driverLatLng.lat, driverLatLng.lng],
      [targetPoint.lat, targetPoint.lng]
    ], {
      color: '#10b981',
